...
class M2T2Inference:
    def __init__(self, cfg=None):
        ...
        # Force the 4 runtime parameters (overriding any CLI overrides).
        self.cfg.eval.checkpoint = str(here / "m2t2.pth")
        self.cfg.eval.data_dir   = str(SCENE_DIR)
    ...
    def load_and_predict(self, data_dir):
        ...
        obj_data = np.load(str(SCENE_DIR / "full_object_pointcloud.npz"))
    ...
    def run(self):
        ...
        output_filename_alt = str(SCENE_DIR / "best_poses.json")
        with open(output_filename_alt, "w") as f:
            json.dump(best_poses, f, indent=2)
        logger.info("Saved '%s'.", output_filename_alt)
        sorted_output_file = str(SCENE_DIR / "sorted_tcp_poses.json")
        self.generate_sorted_tcp_poses(output_filename_alt, sorted_output_file)
        logger.info("Generated '%s'.", sorted_output_file)
        if best_poses["task"] == "pick":
            for i, grasp in enumerate(best_poses["grasps"]):
                logger.info("Pose %d: confidence = %s", i + 1, grasp['confidence'])
        else:
            for i, placement in enumerate(best_poses["placements"]):
                logger.info("Placement %d: confidence = %s", i + 1, placement['confidence'])
        logger.info("M2T2 inference complete.")
